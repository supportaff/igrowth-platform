...
import {
  ...
  Image, Video, Loader2, AlertTriangle, CheckCircle2
} from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const loadIG = useCallback(async (showSpinner = false) => {
    ...
    try {
      ...
      const sessionRes  = await fetch('/api/instagram/session')
      ...
      const insRes  = await fetch('/api/instagram/insights')
      const insData = await insRes.json()

      if (insData.error) {
        // Transient Instagram server error — show connected but with retry prompt
        setIg({ connected: true, error: insData.error, is_transient: insData.is_transient })
      } else {
        setIg({ connected: true, ...insData })
      }
    } catch {
      setIg({ connected: false })
    } finally {
  ...
  // Auto-retry once after 5s if transient error
  useEffect(() => {
  ...
  return (
    <div className="space-y-8">
      ...
      {igLoading ? (
        ...
        // Connected but Instagram API returned a transient error
        <div className="bg-amber-500/8 border border-amber-500/20 rounded-2xl p-5 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-semibold text-amber-300">Instagram account connected</p>
              <p className="text-xs text-white/40 mt-0.5">
                Instagram’s servers are temporarily unavailable (this is on their end). {ig.is_transient ? 'Auto-retrying…' : 'Please try again in a moment.'}
              </p>
            </div>
          </div>
          <button onClick={() => { setRetryCount(0); loadIG(true) }} disabled={refreshing}
            className="flex-shrink-0 flex items-center gap-1.5 text-xs text-amber-300 bg-amber-500/15 border border-amber-500/25 px-3 py-1.5 rounded-lg hover:bg-amber-500/25 transition-colors disabled:opacity-40">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="text-center bg-white/5 border border-white/10 rounded-xl px-4 py-2">
                <p className="text-lg font-extrabold tabular-nums text-violet-300">{fmt(ig.followers ?? 0)}</p>
                <p className="text-xs text-white/35">Followers</p>
              </div>
              <div className="text-center bg-white/5 border border-white/10 rounded-xl px-4 py-2">
                <p className="text-lg font-extrabold tabular-nums text-fuchsia-300">{ig.mediaCount ?? 0}</p>
                <p className="text-xs text-white/35">Posts</p>
              </div>
              <div className="text-center bg-white/5 border border-white/10 rounded-xl px-4 py-2">
                <p className="text-lg font-extrabold tabular-nums text-green-300">{ig.engagementRate ?? '0.00'}%</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Followers',       value: ig.connected && !ig.error ? fmt(ig.followers ?? 0)                                                                         : '—', color: 'text-violet-400' },
          { label: 'Total Posts',     value: ig.connected && !ig.error ? (ig.mediaCount ?? 0).toString()                                                                 : '—', color: 'text-fuchsia-400' },
          { label: 'Engagement Rate', value: ig.connected && !ig.error ? `${ig.engagementRate ?? '0.00'}%`                                                               : '—', color: 'text-green-400' },
          { label: 'Avg Likes/Post',  value: ig.connected && !ig.error && ig.posts?.length
