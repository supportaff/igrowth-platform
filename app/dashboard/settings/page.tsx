...
export default function SettingsPage() {
  ...
  function updateStr(key: keyof FormState, val: string) {
    setForm(f => ({ ...f, [key]: val }))
    setSaved(false)
  }

  function updateBool(key: keyof FormState) {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      {activeTab === 'profile' && (
        <div className="bg-white/4 border border-white/8 rounded-2xl p-6 space-y-5">
          ...
          {([
            { label: 'Full name',            key: 'name'  as keyof FormState, type: 'text',  placeholder: 'Your name' },
            { label: 'Email address',        key: 'email' as keyof FormState, type: 'email', placeholder: 'you@example.com' },
            { label: 'Brand / Business name',key: 'brand' as keyof FormState, type: 'text',  placeholder: 'e.g. Priya Styles Co.' },
          ] as const).map(field => (
            <div key={field.key}>
              ...
              <input
                ...
                onChange={e => updateStr(field.key, e.target.value)}
          ...
          <div>
            ...
            <textarea value={form.bio} onChange={e => updateStr('bio', e.target.value)} rows={3}
      ...
      {activeTab === 'notifications' && (
        <div className="bg-white/4 border border-white/8 rounded-2xl p-6 space-y-4">
          ...
          {([
            { key: 'notifLeads'  as keyof FormState, label: 'New lead alerts',       desc: 'Get notified when a new lead is captured' },
            { key: 'notifDMs'    as keyof FormState, label: 'DM conversation alerts', desc: 'Get notified on new incoming DMs' },
            { key: 'notifWeekly' as keyof FormState, label: 'Weekly summary email',   desc: 'Receive a weekly performance digest' },
